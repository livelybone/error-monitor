  ...
  createClass(ErrorMonitor, [{
    ...
    value: function init() {
      ...
      window.addEventListener('error', function (ev) {
        ...
        if (ev.message) {
          // 脚本错误
          _this.postMsg({ type: 'error-runtime', level: 'warn', message: ev.message, details: { error: error } });
        } else {
          // 资源加载错误
          _this.postMsg({ type: 'error-resource', level: 'warn', message: error.target, details: { error: error } });
        }
      }, true);
    }
  }, {
    ...
    value: function postMsg(_ref2, probability) {
      ...
      if (shouldSend) {
        ...
        var send = function send(pos) {
          _this2.http.post(url, _this2.buildMsg({ type: type, level: level, message: message, position: pos, details: details }), {
    ...
    value: function buildMsg(_ref4) {
      var type = _ref4.type,
          ...
          level = _ref4$level === undefined ? 'error' : _ref4$level,
